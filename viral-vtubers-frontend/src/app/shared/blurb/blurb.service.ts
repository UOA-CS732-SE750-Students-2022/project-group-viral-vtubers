...
export class BlurbService {
  categoryToId: Map<string, string> = new Map<string, string>([
    ...
    ['accessory', '6276debd33169090868b0ec6'],
  ...
  subcategoryToId: Map<string[], string> = new Map<string[], string>([
    ...
    [['accessory', 'tattoos'], '6276e1849bc433f69c0c9274'],
    [['accessory', 'hair-acc'], '6276e1892b1a01f1a0e9cd1d'],
    [['accessory', 'hand-acc'], '6276e18d50227e22ca04bd9b'],
  ...
  subcategoryHashToId: Map<string, string>;
  ...
  constructor() {
    ...
    this.subcategoryHashToId = new Map(
      Array.from(this.subcategoryToId).map(([key, value]) => [
        key.join(),
        value,
      ])
    );
  ...
  getSubcategoryId(category: string, subcategory: string): string {
    return this.subcategoryHashToId.get([category, subcategory].join()) ?? '';
  }
}
